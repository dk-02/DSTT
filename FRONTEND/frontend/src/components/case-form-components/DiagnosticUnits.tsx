...
export const DiagnosticUnits = () => {
    const { caseData, addDU, updateDU, removeDU, setCaseData } = useCaseStore();
    ...
    const calculateSuggestedBudget = () => {
        ...
        caseData.diagnostic_units.forEach(du => {
            idealMoney += du.resources.money || 0;
            let timeInSeconds = du.resources.time || 0;
            if (du.resources.time_unit === 'minutes') timeInSeconds *= 60;
            if (du.resources.time_unit === 'hours') timeInSeconds *= 3600;
            if (du.resources.time_unit === 'days') timeInSeconds *= 86400;
            
            idealTime += timeInSeconds;
        });
    ...
    return (
        ...
        {caseData.diagnostic_units.length > 0 && 
            <p className='mb-5 border rounded-sm border-orange-500 bg-orange-500/20 w-fit px-2 py-1'>{calculateSuggestedBudget()}</p>
        }
        <div className="grid grid-cols-3 gap-4 text-xs">
            <div>
                <label className="block text-gray-300 mb-1 uppercase font-bold tracking-tighter">Budžet (€)</label>
                <input 
                    type="number" 
                    className="w-full p-2 border rounded focus:ring-1 focus:ring-orange-500 outline-none"
                    placeholder='Ostavite prazno za 0'
                    value={caseData.budget.money === 0 ? '' : caseData.budget.money}
                    onChange={(e) => setCaseData({...caseData, budget: {...caseData.budget, money: e.target.value === '' ? 0 : Number(e.target.value)}})}
                />
            </div>
            <div>
                <label className="block text-gray-300 mb-1 uppercase font-bold tracking-tighter">Vremenski budžet</label>
                <input 
                    type="number" 
                    className="w-full p-2 border rounded focus:ring-1 focus:ring-orange-500 outline-none"
                    placeholder='Ostavite prazno za 0'
                    value={caseData.budget.time === 0 ? '' : caseData.budget.time}
                    onChange={(e) => setCaseData({...caseData, budget: {...caseData.budget, time: e.target.value === '' ? 0 : Number(e.target.value)}})}
                />
            </div>
            <div>
                ...
                <select 
                    ...
                    value={caseData.budget.time_unit}
        ...
        <div className="space-y-4">
            {caseData.diagnostic_units.length === 0 ? 
                <div className="w-full h-30 flex justify-center items-center border border-gray-500 rounded-xl">
                    <span className="text-lg text-gray-300">Niste dodali nijednu dijagnostičku jedinicu.</span>
                </div> 
            : 
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-xs">
                        ...
                        <div>
                            ...
                            <input 
                                ...
                                className="w-full p-2 border rounded focus:ring-1 focus:ring-orange-500 outline-none"
                                placeholder='Ostavite prazno za 0'
                                value={du.resources.money === 0 ? '' : du.resources.money}
                                onChange={(e) => updateDU(du.id, { resources: { ...du.resources, money: e.target.value === '' ? 0 : Number(e.target.value) }})}
                            />
                        </div>
                        <div>
                            ...
                            <input 
                                ...
                                className="w-full p-2 border rounded focus:ring-1 focus:ring-orange-500 outline-none"
                                placeholder='Ostavite prazno za 0'
                                value={du.resources.time === 0 ? '' : du.resources.time}
                                onChange={(e) => updateDU(du.id, { resources: { ...du.resources, time: e.target.value === '' ? 0 : Number(e.target.value) }})}
                            />
                        </div>
                        <div>
                            <label className="block text-gray-300 mb-1 uppercase font-bold tracking-tighter">Jedinica</label>
                        ...
                        <div className="pt-4 space-y-4">
                            ...
                            <div className="flex flex-col lg:flex-row gap-3">
                                {du.required_units.map(reqId => {
                                    ...
                                    return (
                                        <div key={reqId} className="lg:w-1/2 bg-gray-200 p-3 rounded-xl border border-gray-200 shadow-sm flex flex-col gap-3">
                                            ...
                                            <div className="flex flex-col text-gray-600">
                                                ...
                                                <span className="mt-3 text-xs font-bold">Opcionalno (ostavite prazno za vrijednost 0):</span>
                                                <div className="flex gap-2">
                                                    <div className="flex-1">
                                                        <label className="text-[11px] font-bold text-gray-400">Kazna novac (€):</label>
                                                        <input 
                                                            type="number"
                                                            min="0"
                                                            className="text-xs p-1 border rounded outline-none w-full bg-gray-100"
                                                            placeholder='npr. 50'
                                                            value={consequence?.penalty_money === 0 ? '' : consequence?.penalty_money}
                                                            onChange={(e) => {
                                                                const newCons = [...du.consequences.filter(c => c.required_id !== reqId)];
                                                                newCons.push({ 
                                                                    required_id: reqId, 
                                                                    ...
                                                                    value: consequence?.value || '',
                                                                    penalty_money: e.target.value === '' ? 0 : Number(e.target.value),
                                                    ...
                                                    <div className="flex-1">
                                                        <label className="text-[11px] font-bold text-gray-400">Kazna vrijeme:</label>
                                                        <input 
                                                            type="number"
                                                            min="0"
                                                            className="text-xs p-1 border rounded outline-none w-full bg-gray-100"
                                                            value={consequence?.penalty_time === 0 ? '' : consequence?.penalty_time}
                                                            placeholder='npr. 30'
                                                            onChange={(e) => {
                                                                const newCons = [...du.consequences.filter(c => c.required_id !== reqId)];
                                                                newCons.push({ 
                                                                    ...
                                                                    penalty_time: e.target.value === '' ? 0 : Number(e.target.value),
                                                                    penalty_time_unit: consequence?.penalty_time_unit || ''
                                                                });
                                                                updateDU(du.id, { consequences: newCons });
                                                            }}
                                                    ...
                                                    <div className="flex-1">
                                                        <label className="text-[11px] font-bold text-gray-400">Jedinica</label>
                                                        <select 
                                                            className="text-xs p-1 border rounded outline-none w-full bg-gray-100"
                                                            value={consequence?.penalty_time_unit}
                                                            onChange={(e) => {
                                                                const newCons = [...du.consequences.filter(c => c.required_id !== reqId)];
                                                                newCons.push({ 
                                                                    required_id: reqId, 
                                                                    ...
                                                                    penalty_money: consequence?.penalty_money || 0,
                                                                    penalty_time: consequence?.penalty_time || 0,
                                                                    penalty_time_unit: e.target.value || ''
                                                                });
                                                                updateDU(du.id, { consequences: newCons });
                                                            }}
                                                        >
            ...
            <Modal isOpen={confirmModalOpen} onClose={() => setConfirmModalOpen(false)} title='Obrisati DU?'>
                <div className='flex justify-center w-full'>
                    <button 
                        ...
                        className={"bg-gray-400 font-bold w-1/4 p-3 hover:cursor-pointer rounded-lg flex items-center justify-center gap-2 border transition"}
                    >
                        Odustani
                    </button>
                    <button 
                        onClick={() => handleRemoveDU(DUToDeleteId)}
                        className={"bg-red-600 font-bold w-1/4 p-3 hover:cursor-pointer rounded-lg flex items-center justify-center gap-2 border transition"}
                    >
                        Potvrdi
                    </button>
                </div>
